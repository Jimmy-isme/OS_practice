...
void uart_init() {
    ...
    *GPPUDCLK0 = 0;
    ...
    // 9. 啟用 mini UART 中斷（RX先開 TX先別）
    *AUX_MU_IER_REG = 1;  // bit 0 = 1 (RX 中斷), bit 1 = 1 (TX 中斷)

    // 10. 啟用第一級中斷控制器的 bit 29 (mini UART IRQ)
    *ENABLE_IRQS1 |= (1 << 29);
...
void uart_send(unsigned int c) {
    while (!(*AUX_MU_LSR_REG & 0x20));  // 等待 TX 可用(TX FIFO為空就跳出)
    *AUX_MU_IO_REG = c;
}

// 接收單一字元
char uart_recv() {
    while (!(*AUX_MU_LSR_REG & 0x01));  // 等待 RX 有資料(RX FIFO沒空就跳出)
