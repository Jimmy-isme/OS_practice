...
void shell_start() 
{
    ...
    while (1) 
    {
        ...
        while (1) 
        {
            ...
            else if ((c == 8 || c == 127) && index > 0)  // Backspace (ASCII 8) 或 DEL (ASCII 127) 
            {  // 處理 Backspace
                index--;
                uart_puts("\b \b");  // 刪除字元
            } 
        ...
        if (string_compare(buffer, "help") == 0) 
        {
            ...
            uart_puts("  dtb - Get the hardware's information v2\n");
        } 
        ...
        else if (string_starts_with(buffer, "dtb"))
        {
            extern void *_dtb_ptr;
            uart_puts("Parsing Device Tree...\n");
            if (fdt_traverse(print_dtb, _dtb_ptr) < 0)
            {
                uart_puts("Failed to parse DTB!\n");
            }
        }
