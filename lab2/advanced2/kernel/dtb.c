...
int parse_struct (fdt_callback cb, uintptr_t cur_ptr, uintptr_t strings_ptr, uint32_t totalsize) {
    ...
    while(cur_ptr < end_ptr) {
        ...
        switch(token) {
            ...
            case FDT_END:
                cb(token, NULL, NULL, 0);
                uart_puts("DTB parsing completed.\n");
                return 0;
            default:
                return -1;
        }
...
int fdt_traverse(fdt_callback cb, void * _dtb) {
    ...
    if (struct_ptr >= dtb_ptr + totalsize || strings_ptr >= dtb_ptr + totalsize) 
    {
        uart_puts("Error: DTB structure or strings pointer out of range!\n");
        return -1;
    }
...
void get_cpio_addr(int token, const char* name, const void* data, uint32_t size) {
    UNUSED(size);
    if (token == FDT_PROP && name != NULL && string_compare((char *)name, "linux,initrd-start") == 0) {
