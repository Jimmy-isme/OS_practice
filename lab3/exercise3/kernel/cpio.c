...
void cpio_cat(char *filename) {
    char *target = cpio_findFile(filename);
    if (target) {
...
char *cpio_findFileAddress(char *filename) {
    char *addr = cpio_addr;  // cpio 的起始記憶體位址

    while (string_compare((char *)(addr + sizeof(struct cpio_header)), "TRAILER!!!") != 0) {
        struct cpio_header *header = (struct cpio_header *)addr;
        char *file_name = addr + sizeof(struct cpio_header);

        if (string_compare(file_name, filename) == 0) {
            unsigned int namesize = hex_to_int(header->c_namesize, sizeof(header->c_namesize));
            unsigned int headerPathname_size = sizeof(struct cpio_header) + namesize;
            headerPathname_size = (headerPathname_size + 3) & ~3;  // 4-byte alignment

            return addr + headerPathname_size;  // 回傳檔案內容的記憶體位址
        }
...
unsigned int cpio_get_file_size(char *filename) {
    ...
    while (string_compare((char *)(addr + sizeof(struct cpio_header)), "TRAILER!!!") != 0) {
        ...
        if (string_compare(file_name, filename) == 0) {
            return hex_to_int(header->c_filesize, sizeof(header->c_filesize)); // 回傳檔案大小
        }
